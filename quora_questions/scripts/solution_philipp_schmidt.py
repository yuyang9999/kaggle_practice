...
tsne = TSNE(n_components=3, init='random', random_state=101, method='barnes_hut', n_iter=200,
            verbose=2, angle=0.5).fit_transform(diff_encodings.toarray())

trace1 = go.Scatter3d(x=tsne[:, 0],
                      y=tsne[:, 1],
                      z=tsne[:, 2],
                      mode='markers',
                      marker=dict(
...
sns.violinplot(x='is_duplidate', y='word_share', data=df[0:50000])
plt.subplot(1,2,2)
...
df_subsampled = df_subsampled[df_subsampled['q_n_words_avvg'] < 20]
df_subsampled.head()
...
figure['layout']['yaxis'] = {'range':[0, 200], 'title':'Q2 length'}
        ...
        'bottons':[
            {
                'args':[None, {'frame':{'duration':300, 'redraw': False},
                               'fromcurrent': True, 'transition':{'duration': 300, 'easing':'quatratic-in-out'}}],
                'label': 'Play',
                'method': 'animate'
            },
            {
